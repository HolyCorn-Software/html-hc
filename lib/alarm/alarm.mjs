...
/**
 * @template Type
 * @extends htmlhc.lib.alarm.AlarmObjectClass<Type>
 */
export default class AlarmObject extends EventTarget {
     ...
     * 
     * @param {object} param0 
     * @param {boolean} param0.is_array
     * @param {AbortSignal} param0.abortSignal
     */
    constructor({ is_array, data = {}, abortSignal } = {}) {
        super();

        if (abortSignal) {
            this.addEventListener = (type, cb, opts) => {
                EventTarget.prototype.addEventListener.call(this, type, cb, { signal: abortSignal, ...opts })
            }
        }
     ...
     * @template FieldName
     * @param {FieldName|htmlhc.lib.alarm.RecurDotNames<Type>} field 
     * @returns {Promise<Type[FieldName]>}
     */
     ...
     * @param {htmlhc.lib.alarm.EventNames<Type>} type 
     * @param {()=>void} fxn 
     * @param {AddEventListenerOptions} options 
     * @param {boolean} immediate If set, the function will be called immediately
     */
    addEventListener(type, fxn, options, immediate) {
        /** @type {(eventName: htmlhc.lib.alarm.EventNames<Type>, cb: function)} */ this.removeEventListener
...
function recursive_dispatch(original_interface, value, path) {
    ...
    if (!value || Reflect.getPrototypeOf(value) != Reflect.getPrototypeOf({})) {
        // Only perform recursive dispatch with "purely" fabricated objects.
        // In this way, objects like HTMLElement, and other complex ones can be nicely excluded
        return;
    }
