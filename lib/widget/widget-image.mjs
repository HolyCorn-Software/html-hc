            ...
            set: async (url0) => {
                ...
                if (mode == 'inline') {
                    const getImageNormally = async () => {
                        ...
                        iconElm.appendChild(
                            hc.spawn({ tag: 'img', attributes: { src: url.href, loading: 'lazy', crossorigin: 'anonymous' }, classes: ['hc-htmlhc-widget-image'] })
                        );
                    }
                    ...
                    iconElm.style.setProperty('background-image', `url('${url.href}'), url('${fallback}')`)
                }
