...
/**
 * @returns {import("./widget.js").ExtendedHTML}
 */
...
/** @function @returns {import("./widget.js").ExtendedHTML} */
export let $ = document.$.bind(document);
...
/**
 * @returns {import("./widget.mjs").ExtendedHTML}
 */
...
/**
 * @deprecated
 * Use setupViewport() instead to make the website fit to mobile devices
 */
export let makeScalable = setupViewport
...
export function importJS(src) {
	return new Promise((y, n) => {
		...
		script.onload = y;
		script.onerror = (e) => {
			n(new Error(`Internet Connection Error. You might need to reload the page.`));
		}
	})
};
 ...
 */
export async function importModuleCSS(importUrl, cssFile) {
	let finalCSSUrl;
	try {
		if (!importUrl) {
...
export function getCaller(offset = 0) {
	return /(http.+):\d+:\d+/.exec(new Error().stack.split('\n')[offset + 1 + 1])?.[1]
}
