...
/**
 * @returns {import('./types.js').ExtendedHTML<import('./index.mjs').Widget>}
 */
...
export function getCaller(offset = 0, ignoreRegExps = []) {
	...
	lines = lines.filter(x => !([...ignoreRegExps, ...defaultIgnores].some(reg => reg.test(x))))
	const line = /([a-z]+:\/\/.+)(?::\d+:\d+)/.exec(lines[offset + 1])?.[1]
	if (!/http/.test(line)) {
