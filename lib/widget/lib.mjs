...
/**
 * This method finds an HTMLElement that is a parent of the target element, using the selector
 * @param {HTMLElement} html 
 * @param {string} selector 
 * @returns 
 */
export function ancestor(html, selector) { //Nothing I love more than Logic!
	let ancestor = html;
...
/**
 * This method waits till an event is called, or a time period is elapsed.
 * @param {HTMLElement} html 
 * @param {(keyof HTMLElementEventMap)[]} events The event we're listening for
 * @param {object} param1 Options
 * @param {number} param1.timeout The maximum time to wait
 */
export const waitForDOMEvents = (html, events, { timeout }) => {
	return new Promise((resolve) => {
		const done = () => {
			resolve()
			for (const event of events) {
				html.removeEventListener(event, done)
			}
			clearTimeout(timeoutKey)
		}
		const timeoutKey = setTimeout(done, timeout || Infinity)
		for (const event of events) {
			html.addEventListener(event, done)
		}
	})
}
...
/**
 * This method returns a HH:MM, according to the user's preferences
 * @param {Date} date 
 * @returns {string}
 */
export const toTimeString = (date) => {
	const pad = (numb) => `${numb}`.padStart(2, '0')
 ...
 */
let setupViewport = window.makeScalable = function () {
	document.head.spawn({
		tag: 'meta',
		name: 'viewport',
...
export function importModuleCSS(importUrl, cssFile) {
	return new Promise((resolve, reject) => {
		...
		let moduleCSSUrl = new URL(new URL(importUrl).pathname.split('/').reverse()[0].split('.').slice(0, -1).join('.') + ".css", importUrl).href
		try {
			finalCSSUrl = new URL(cssFile || moduleCSSUrl, moduleCSSUrl).href;
		} catch (e) {
			console.log(`Could not import ${importUrl}, with specific file ${cssFile}, after computing module css url to be: ${moduleCSSUrl}`)
			throw e
		}
		...
		addCSS(finalCSSUrl).then(resolve).catch(() => undefined)
...
window.WidgetEvent = class extends CustomEvent {
	constructor() {
		super(arguments[0], { bubbles: true, ...arguments[1] });
