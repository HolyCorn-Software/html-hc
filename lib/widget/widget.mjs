...
export class __Widget extends EventTarget {
    ...
    waitTillDOMAttached() {
        return new Promise((resolve) => {
            ...
            const check = () => {
                ...
                if (this.destroySignal.aborted || this.html?.isConnected) {
                    try {
                        mut.disconnect()
                    } catch { }
                    return resolve() || true
    ...
    async destroy() {
        ...
        this[destroy].signal.aborted || this[destroy].abort()
    }
    ...
    set html(value) {
        ...
        value.addEventListener = (...args) => {
            // Whenever an event listener is attached to the HTML, let's make sure it is auto-removed after destroy
            return EventTarget.prototype.addEventListener.call(value,
                args[0],
                args[1],
                {
                    ...args[2],
                    signal: args[2]?.signal ? (() => {
                        ...
                        this.destroySignal.addEventListener('abort', () => megaDestroy.abort(), { once: true, signal: megaDestroy.signal })
     ...
     * @param {'innerHTML'|'class'|'attribute'|'inputValue'|'innerText'} type How does the property change the element. Is it by toggling a class, or changing it's innerHTML or something else ?
     ...
     * @param {htmlhc.lib.widget.GetWidgetKeys<this>} param0.property
    ...
    defineImageProperty({ selector, property, cwd, mode, fallback }) {
        return defineImageProperty.call(this, property, selector, cwd, mode, fallback)
    }
     ...
     * 
     * @template {htmlhc.lib.widget.OptionalKeys<this, Target>} PropertyName
     * @template DataWidget
     ...
     * @template {htmlhc.lib.widget.OptionalKeys<this, Target, []>} Property
     * @template Target
     * @template DataWidget
     * @param { htmlhc.lib.widget.WidgetPropertyArgs<htmlhc.lib.widget.OR<Target, this>, Property, "plural", DataWidget>} param0 
...
const originalAbrt = AbortController.prototype.abort
Reflect.defineProperty(AbortController.prototype, 'abort', {
    get: function () {
        return originalAbrt.bind(this)
    }
})
